...
class ParticulesBuilders {
    ...
    public static function randParticule(gpemit:GridPosition,speedRange:Float,lifetime:Float,?body:Bool=false,?fric:Float=0,?gr:Float=0) {
            
        var pa  = new ParticulesComponent(lifetime,body,0.001,gr);
        ...
        var vc  = new VelocityComponent(body);
        var cl  = new CollisionsListener();
        var ent = new echoes.Entity().add(pa,bmp,gp,vas,vc,cl);
        return ent;
    }

    public static function smokeParticule(gpemit:GridPosition,dirX:Float,dirY:Float,lifetime:Float,?body:Bool=false,?fric:Float=0,?gr:Float=0) {
        var pa  = new ParticulesComponent(lifetime,body,fric,gr);
        var bmp = new BitmapComponent(Assets.smoke,0.5,null,SmokeShader);
        var gp  = new GridPosition(gpemit.cx,gpemit.cy,gpemit.xr,gpemit.yr);
